...
#include <isl/isl_schedule_node_private.h>
...
static __isl_give isl_schedule_node *try_overlapped_tile(struct gpu_gen *gen,
	__isl_take isl_schedule_node *node)
{
	int tile_len, block_len, after_mapping;
	...
	after_mapping = 1;
	node = overlapped_tile(node, gen->prog->scop, sizes, block_size, block_len, after_mapping);
	node = isl_schedule_node_band_split(node, 1);
	node = isl_schedule_node_child(node, 0);
	if (!gen->options->multi_level_overlapped && n > 2) {
		node = isl_schedule_node_band_split(node, 1);
...
{
    struct ppcg_mark_outer_data *user_data = user;

	struct gpu_gen *gen = user_data->gen;
	struct ppcg_scop *scop = user_data->scop;
	int outer;
	int scale;
	int tile_len;
	int *tile_size;
...
{
    struct ppcg_mark_outer_data user;
    user.gen = gen;
    user.scop = scop;
	return isl_schedule_node_map_descendant_bottom_up(node,
						&mark_outer_permutable, (void *)(&user));
}
	...
	__isl_take isl_schedule *schedule, struct ppcg_scop *scop)
{
	...
	node = mark_kernels(gen, node, scop);
	...
	struct ppcg_options *options)
{
	...
	if (any_permutable < 0 || !any_permutable) {
		...
		schedule = map_to_device(gen, schedule, scop);
